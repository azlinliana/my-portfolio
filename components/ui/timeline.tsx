        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              {/* Desktop timeline date (left side) */}
              <h3 className="hidden md:block text-md md:pl-20 md:text-lg font-medium text-white">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full text-white">
              {/* Mobile timeline date (above content) */}
              <h3 className="md:hidden block text-base mb-4 text-left font-medium">
                {item.title}
              </h3>

              <div className="text-sm sm:text-base md:text-lg font-normal text-neutral-300 dark:text-neutral-300 leading-relaxed text-justify">
                {item.content}
              </div>
            </div>
          </div>
